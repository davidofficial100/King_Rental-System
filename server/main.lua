...
RentalSystem.RentVehicle = function(playerId, vehicleModel, rentalDays, hasInsurance)
    ...
    if type(rentalDays) ~= 'number' or rentalDays < 1 then
        return false, 'Invalid rental days'
    end
    
    -- Validate player exists
    local player = GetPlayer(playerId)
    if not player then
        return false, 'Player not found on server'
    end
    
    -- Validate vehicle model
    if not vehicleModel or vehicleModel:len() == 0 then
        return false, 'Invalid vehicle model'
    end
    ...
    local maxDays = 0
    for _, duration in ipairs(Config.RentalDurations or {}) do
        if duration.days and duration.days == rentalDays then
            durationValid = true
            break
        end
        maxDays = math.max(maxDays, duration.days or 0)
    end
    
    if not durationValid then
        return false, 'Invalid rental duration. Max days: ' .. maxDays
    end
...
RentalSystem.ReturnVehicle = function(playerId, vehicleData)
    -- Safety: Validate playerId
    if type(playerId) ~= 'number' or playerId <= 0 then
        return false, 'Invalid player ID'
    end
    
    -- Get active rental with safety check
    local rental = nil
    if PlayerRentals and PlayerRentals[playerId] then
        rental = PlayerRentals[playerId]
    elseif Database and Database.GetPlayerActiveRental then
        rental = Database.GetPlayerActiveRental(playerId)
    end
    ...
    -- Safety: Validate rental data
    if not rental.startTime or not rental.rentalDays then
        Utils.Error('Rental data corrupted for player ' .. playerId)
        return false, 'Rental data error'
    end
    
    -- Calculate charges safely
    local currentTime = os.time()
    local rentalDuration = tonumber(rental.rentalDays) or 1
    local endTime = rental.startTime + (rentalDuration * 86400)
    local timeDiff = currentTime - endTime
    local damageCharge = 0
    
    -- Late return fine with safety check
    local lateCharge = 0
    local lateReturnFine = (Config and Config.Fines and Config.Fines.lateReturn) or 100
    if timeDiff > 0 then
        local lateDays = math.ceil(timeDiff / 86400)
        lateCharge = lateDays * lateReturnFine
    end
