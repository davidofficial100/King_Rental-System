...
CreateThread(function()
    ...
    Utils.Success('Client rental system initialized')
end)
...
function SpawnLocationNPC(location)
    ...
    local npc = CreatePed(4, GetHashKey(location.npcModel), location.coords.x, location.coords.y, location.coords.z, location.npcHeading, true, false)
    
    if npc == 0 then
        Utils.Error('Failed to spawn NPC at ' .. location.name)
        return
    end
    ...
    Utils.Debug('Spawned NPC at: ' .. location.name)
end
